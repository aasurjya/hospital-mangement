...
async function loadPlatformStaffGuard(hospitalId: string, staffId: string) {
...
export async function updatePlatformStaffAction(
  hospitalId: string,
  staffId: string,
  ...
  const { supabase, staff } = await loadPlatformStaffGuard(hospitalId, staffId)
  ...
  if (parsed.data.role !== staff.role) {
    ...
    if (authError) {
      console.error('[platform staff] Failed to sync role in app_metadata:', authError.message)
    }
    await writeAuditLog({
      hospitalId,
      actorId: ctx.userId,
...
export async function resetPlatformStaffPasswordAction(
  hospitalId: string,
  staffId: string
