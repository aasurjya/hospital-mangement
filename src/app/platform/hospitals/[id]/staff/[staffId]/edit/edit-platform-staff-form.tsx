...
import {
  updatePlatformStaffAction,
  togglePlatformStaffStatusAction,
  resetPlatformStaffPasswordAction,
...
export function EditPlatformStaffForm({ hospitalId, staffId, initialData, departments }: Props) {
  ...
  const isHospitalAdmin = initialData.role === 'HOSPITAL_ADMIN'
  const isClinicalRole = CLINICAL_ROLES.includes(selectedRole)

  // 1. Profile update
  const boundUpdate = updatePlatformStaffAction.bind(null, hospitalId, staffId)
  ...
  async function handleToggle() {
    ...
    const result = await togglePlatformStaffStatusAction(hospitalId, staffId, !isActive)
  ...
  async function handleReset() {
    ...
    const result = await resetPlatformStaffPasswordAction(hospitalId, staffId)
      ...
      <form action={profileAction} className="space-y-6">
            ...
            <div className="space-y-4 mt-2">
              ...
              <Field label="Qualifications" name="qualifications" error={fieldError(profileState, 'qualifications')}>
                <input id="qualifications" name="qualifications" type="text" defaultValue={initialData.qualifications}
                  disabled={profilePending} placeholder="e.g. MBBS, MD" className={inputCls} />
              </Field>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={profilePending}
            className="rounded-md bg-primary-600 px-5 py-2 text-sm font-medium text-white hover:bg-primary-700 disabled:opacity-50 min-h-[44px]">
            {profilePending ? 'Saving\u2026' : 'Save changes'}
          </button>
          <a href={`/platform/hospitals/${hospitalId}`} className="text-sm text-neutral-500 hover:text-neutral-700 min-h-[44px] inline-flex items-center">
            Cancel
          </a>
        </div>
      </form>
